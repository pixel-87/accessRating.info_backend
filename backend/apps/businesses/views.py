    ...
    def qr_code(self, request, pk=None):
        ...
        try:
            ...
            response = HttpResponse(
                qr_image_buffer.getvalue(), content_type="image/png"
            )
            response["Content-Disposition"] = (
                f'inline; filename="qr_code_{business.id}.png"'
            )
            return response
        except Exception:
            logging.exception(
                "Failed to generate QR code for business ID %s", business.id
            )
            return Response(
                {"error": "Failed to generate QR code due to an internal error."},
                status=500,
            )
